...
int changeToGuided(){
  ...
  mavros_msgs::SetMode srv_mode_guided;
  srv_mode_guided.request.custom_mode = "GUIDED";
  if (set_mode_client.call(srv_mode_guided) && srv_mode_guided.response.mode_sent){
    ROS_INFO("mode changed %d", srv_mode_guided.response.mode_sent);
  	//current_state_g.mode = "GUIDED"
  }
  else
  {
    ROS_ERROR("Failed mode change");
    return -1;
  }
